  ...
  async function loadUserInfo(uid) {
    try {
      ...
      if (userSnap.exists()) {
        ...
        // Use your database field names (e.g., FirstName, LastName, etc.)
  ...
  async function loadSavedPaymentMethods(uid) {
    try {
      // Payment methods are stored in a subcollection at users/{uid}/paymentMethods
  ...
  async function loadCartItems() {
    ...
    // Fetch product details from "listings" collection
  ...
  function renderOrderSummary(products) {
    ...
    products.forEach(product => {
      ...
      div.innerHTML = `<span>${product.name} (x${product.quantity})</span><span>${formatCurrency(itemTotal)}</span>`;
  ...
  function setDeliveryDate() {
    ...
    document.getElementById('deliveryDate').textContent =
      deliveryStart.toLocaleDateString() + ' - ' + deliveryEnd.toLocaleDateString();
  }
  setDeliveryDate();
  ...
  document.getElementById('toConfirmation').addEventListener('click', () => {
    if (validateStep(2)) {
      ...
      document.getElementById('orderEmail').textContent = document.getElementById('email').value;
      currentStep = 3;
      updateFormSteps();
      updateProgress();
    }
  ...
  function validateStep(stepNumber) {
    const currentFormStep = form.querySelector(`.form-step[data-step="${stepNumber}"]`);
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (paymentMethod === 'card-new') {
      ...
      if (document.getElementById('rememberCard').checked) {
        ...
        // Save as a new document in the paymentMethods subcollection
    ...
    try {
      ...
      for (const sellerId in ordersBySeller) {
        ...
        // Generate a unique order ID (e.g., using timestamp and sellerId)
  ...
  async function deleteCartItems(uid) {
    try {
      ...
      // Subtract the total quantity from the user's aggregated cartCount
